...
# display list of celex codes on demand

# split-sentences.exe konzol nélküli verzió (pp --gui -o foo.exe bar.pl -x)
# segmenter msg

# ERROR: / try again; csekkolni a .pl-ben
...
my @inputfile;			# array that holds the filepath of all input files
...
# debug window, disable for release
	...
	my $doStdout = sub {
		if( $Qout->pending ) {
			...
			# $lb->see( 'end' );		# reloads the listbox and shows the last entries in case they scrolled off the end
...
################################################
			...
			$tmx_langcode_1_def = $3;
			$tmx_langcode_2_def = $4;
			$tmx_settings{creationid} = $5;
...
##########################################
			}	elsif ( $output =~ /Number of languages\?/ ) {
				
				$langs_fullnames[0] = $lang_1_iso_def;
				$langs_fullnames[1] = $lang_2_iso_def;
				$langs_fullnames[2] = "Spanish";
				...
				my $frm_getinfo = $mw -> Frame() -> pack(-fill => 'both', -expand => 1); #scrolled in case of small screen or many languages
				# my $frm_getinfo = $mw -> Scrolled('Frame', -scrollbars => 'osoe') -> pack(-fill => 'both', -expand => 1); #scrolled in case of small screen or many languages
				$frm_getinfo-> Label(-text => "Specify the languages of your texts:\n", -font=>'bold')->pack (-pady => 10);
				...
				my $changeno = $frm_langs_no -> BrowseEntry(
					...
					-browsecmd => sub {
						...
						$frm_langs_all = $frm_langs -> Frame() -> pack(-expand => 1, -fill => 'both');	# create the frame again
						&$apply_changeno;
						}
				)->pack();# pack(-side => 'left');
				$changeno->insert('end', (2 .. 99));

				$frm_langs_all = $frm_langs -> Frame() -> pack(-expand => 1, -fill => 'both'); # frame so that we can destroy all the $frm_langs[$i] frames together
				
				
				&$apply_changeno; # build entry boxes once when the window loads (rebuilt when $no is changed)
				
				
				$frm_getinfo -> Label(-text => "\nNote: you can change the default languages by editing LF_aligner_setup.txt",)->pack;
...
#############################################
				...
				my $frm_web = $mw -> Frame() -> pack(-expand => 1, -fill => 'both');
...
#############################################
				...
				$inputfile[$i] = $mw ->getOpenFile(
									...
									$labeltext[$i] = $inputfile[$i];										# this will be displayed by the autoupdated label
...
# Note: The files need to be in the same folder.", )->pack (-anchor => 'w');
				...
				# enqueue the first file, do the rest automatically
...
#############################################
				...
				my $ii = $1;
				my $i = $ii - 1;
				$Qin->enqueue($inputfile[$i]);	 # the file was picked previously, we just feed it to the .pl from the array here


#############################################
		} elsif ( $output =~ /File \d+ not found.*Try again/ ) {
				# pop up a window that reports the $output error msg
...
#   } elsif ( $output =~ /Input file stats:/ ) { }	#do
#   } elsif ( $output =~ /Running the Chinese\/Japanese segmenter on the (..) text/ ) { }	#do


#############################################
			} elsif ( $output =~ /Pdf to txt conversion done./ ){
				...
				my $reviewpdf = $mw->Dialog(-title => 'Review pdf', 
				   ...
				   -default_button => 'Close', -buttons => ['View txt files', 'Next'])->Show( );
				...
				my $buttnext = $frm_pdf -> Button(-text=>"Next", -command =>sub {$Qin->enqueue( "Move on." );$frm_pdf->destroy;}) -> pack(-side => 'bottom', -anchor=> 'se', -padx => 3, -pady => 3);
				$buttnext->focus( -force );
...
$line_no_seg[$i] = $3;
				...
				my $frm_revert = $mw -> Frame() -> pack(-expand => 1, -fill => 'both');
...
#############################################
				...
				$frm_create_tmx -> Label(-text => "\nNote: you'll need a TMX file for importing your aligned texts into a CAT tool such as Trados", )->pack (-anchor => 'w');
...
#############################################
				} elsif ( $output =~ /Default creationdate: (.*Z)/ ){ # default note not available at startup with all other defaults because it' set by main script based on filenames etc.
...
#############################################
				...
				my $frm_tmx_settings = $mw -> Frame() -> pack(-expand => 1, -fill => 'both');
				
				$frm_tmx_settings -> Label(-text => "Please provide the settings for the TMX\n", -font=>'bold')->pack (-pady => 10);
				...
				my $tmxnote_label = $frm_tmx_note -> Label(-text=>"Note to be added to each TU: ")->pack(-anchor => 'w', -side => 'left');
				my $tmxnote_entry = $frm_tmx_settings_right -> Entry( -width => 25, -textvariable => \$tmx_settings{note},)->pack(-anchor => 'w',);
				...
				$frm_tmx_note -> Checkbutton(
									...
									-offvalue => 'disabled',
									-command  => sub {
														$tmxnote_label->configure( -state => $toggle);
														$tmxnote_entry->configure( -state => $toggle);
														if ($toggle eq "disabled") {$tmx_settings{note} = ""}},
				...
				$frm_tmx_settings -> Label(-text => "\nNote: CAT tools tend to be picky about what language codes they accept in TMX files.\nYou might have to add a country code to the language code, i.e. EN-GB or EN-US instead of EN etc.\nIf in doubt, export a TM into TMX with the CAT tool you will be using and check the codes it uses.\nAlternatively, you can take a stab in the dark and hope for the best.",)->pack (-anchor => 'w');


				
				my $buttnext = $frm_tmx_settings -> Button(-text=>"Next", -command =>sub {$Qin->enqueue( $tmx_langcodes[0] );$frm_tmx_settings->destroy;}) -> pack(-side => 'bottom', -anchor=> 'se', -padx => 3, -pady => 3);
				$buttnext->focus( -force );
...
$tmx_settings{note} = "none" if $tmx_settings{note} eq ""; # test
...
#############################################
				...
				my $quit = $mw->Dialog(	-title => "The end",
										-text => "The aligner has terminated successfully.\nClick OK to exit.", 
										-default_button => 'OK', -buttons => ['OK'])->Show( );
				if ($quit eq 'OK') {
					$Qin->enqueue("Done");
				}
